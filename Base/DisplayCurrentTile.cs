...
namespace UnityUserInput {
    ...
    public class DisplayCurrentTile : Action {
                    ...
                    if (currentTile.worldCoord.x > tilesToDisplay.x / 2 && currentTile.worldCoord.x < numTilesX - tilesToDisplay.x / 2 &&  currentTile.worldCoord.z > tilesToDisplay.z / 2 && currentTile.worldCoord.z < numTilesZ - tilesToDisplay.z / 2) {
                        ...
                        for (int z = currentTile.worldCoord.z - zOffset; z <= currentTile.worldCoord.z + zOffset; z++) {
                            for (int x = currentTile.worldCoord.x - xOffset; x <= currentTile.worldCoord.x + xOffset; x++) {
                                ...
                                GridTile neighbour;
                                if (!gameInputData.isMouse0) { // not selecting tiles
                                    neighbour = gridData.GetTileFromWorldCoord(new Coord(x, currentTile.worldCoord.y, z));
                                } else {
                                    GridTile mouseDownTile = gridData.GetTileFromPosition(gameInputData.mouseDownPosition);
                                    neighbour = gridData.GetTileFromWorldCoord(new Coord(x, mouseDownTile.worldCoord.y, z));
                                }
        ...
        public void SetTileSize (int mode) {
            if (mode == 0) {
                tilesToDisplay = new Vector3Int(1,1,1);
