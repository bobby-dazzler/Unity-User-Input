    ...
    public class SelectTilesAct : Action {
        ...
        public override void Act (StateController controller) {
            ...
            if (mouseDownTile != currentTile || gameInputData.selectedTiles.Count == 0) {
                ...
                //gameInputData.minY = Mathf.Min(mouseDownWorldCoord.y, currentWorldCoord.y); // prevent selecting multiple y coords for now
                gameInputData.minY = mouseDownWorldCoord.y;
                ...
                gameInputData.maxY = gameInputData.minY;
                ...
                for (int y = gameInputData.minY; y <= gameInputData.maxY; y++) {
                    for (int z = gameInputData.minZ; z <= gameInputData.maxZ; z++) {
                        for (int x = gameInputData.minX; x <= gameInputData.maxX; x++) {
                            ...
                            if (tile.tileType.contentName == "Empty") {
                                gridData.tileFactory.ChangeTileType(tile, 4, true);
                                selectedTileTypeTiles.Add(tile);
                            } 
